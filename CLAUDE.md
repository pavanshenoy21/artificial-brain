...
## Current state
- Milestones 1–6 done (see git log). Next: milestone 7 (forms + GitHub sync).
- Rust (`src-tauri/src/`): `vault.rs` (files, frontmatter, lobes.json, trash), `index.rs` (SQLite: items, tags,
  ...
  `embed.rs` (vectors, background worker, similar links, semantic search), `commands.rs`, `state.rs`, `error.rs`,
...
- Frontend: `main.js` (bootstrap, keys), `capture.html` + `capture.js` (quick-capture window), `state.js` (shared state + events), `api.js` (+ `mock/backend.js`),
  ...
  `editor/` (item tab: title, CodeMirror editor `editor.js`, reading view `markdown.js`), `lib/` (types, lobes, wikilinks, prefs, rank), `settings/view.js` (Settings tab), `theme.css` + `app.css`.
