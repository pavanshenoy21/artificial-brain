...
- Big-vault performance (tested with 3,000 notes / 6,000 links): shared-tag similarity uses a tag→items index, ignores tags on >15% of items and is cached (6.5 s → under 0.1 s; it ran on every autosave and froze the app). Links are drawn as two merged line batches with per-vertex colour/alpha instead of one object per link; node geometry is shared; labels are created lazily; one outline mesh; nodes are only transparent while faded. The graph stops rendering when idle (0 frames/s) and wakes on interaction, camera moves, layout ticks and data changes.
- A lost WebGL context (GPU driver reset, e.g. Mesa/Zink VK_ERROR_DEVICE_LOST) no longer freezes the graph: it pauses and shows a Reload button; the rest of the app keeps working. README lists driver workarounds.
- Top-level folders become lobes automatically (`folder-<slug>` ids, a `folder` field in `.brain/lobes.json`, a distinct colour from a 16-colour palette). Items without their own `lobe:` take their folder's lobe at read time (`lobe_from: folder`); files are never rewritten. A folder lobe deleted in Settings is remembered in `.brain/folders-ignored.json` and not recreated. The app's own type folders don't become lobes. Empty lobes are hidden from the filter list.
